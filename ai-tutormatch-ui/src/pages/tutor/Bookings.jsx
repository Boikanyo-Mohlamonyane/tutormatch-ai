...
export default function TutorBookings() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const data = await tutorApi.getTutorBookings(user.id);
      setBookings(data || []);
    } catch (err) {
  ...
  const act = async (booking, action) => {
    ...
    try {
      if (action === "approve") {
        await tutorApi.approveBooking(id);
        toast.success("Session approved.");
      } else {
        await tutorApi.rejectBooking(id);
  ...
  return (
    <div>
      <PageHead
        ...
        title="Bookings"
        description="Respond to session requests from your students."
        action={
